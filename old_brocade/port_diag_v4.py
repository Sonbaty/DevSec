...
def get_info(IP, any):
    ...
    for interface in interface_list:
        if '1/' in interface:
            ...
            port_errors = connection.send_command(
                f'sh int ethernet {interface} | i error')
            port_uptime = connection.send_command(
                f'sh int ethernet {interface} | i minute')
            attached_device = connection.send_command(
                f'sh lldp neighbors detail ports ethernet {interface} | i System description')
        ...
        if error_check == True:
            ...
            if '1/1/' in interface:
                ...
                connection.send_command(
                    f'phy cable-diagnostics tdr {interface}')
                tdr_result = connection.send_command(
                    f' sh cable-diagnostics tdr {interface}')
